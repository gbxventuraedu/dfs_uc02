...
--laboratorio-pag-1011-pag-102
-- LaboratórioPara realizar esse laboratório, utilize o banco de dados DB_CDS. 
-- 1.Exiba quantos pedidos cada cliente fez. Resposta: 
SELECT
    cli.nome_cli AS 'Cliente',
    COUNT(ped.num_ped) AS 'Quantidade de Pedidos'
FROM
    tbClientes AS cli
    INNER JOIN tbPedidos AS ped ON cli.cod_cli = ped.cod_cli
GROUP BY
...
-- 2.Exiba quantos CDs possui cada categoria. Resposta: 
SELECT
    cat.nome_cat AS 'Categoria',
    COUNT(tit.cod_tit) AS 'Quantidade de CDs'
FROM
    tbCategorias AS cat
    INNER JOIN tbTitulos AS tit ON cat.cod_cat = tit.cod_cat
GROUP BY
    cat.nome_cat;

-- 3.Exiba quantos CDs possui cada gravadora. Resposta: 
SELECT
    grav.nome_grav AS 'Gravadora',
    COUNT(tit.cod_tit) AS 'Quantidade de CDs'
FROM
    tbGravadoras AS grav
    INNER JOIN tbTitulos AS tit ON grav.cod_grav = tit.cod_grav
GROUP BY
    grav.nome_grav;

-- 4.Exiba quantos pedidos cada funcionário atendeu. Resposta: 
SELECT
    func.nome_func AS 'Funcionário',
    COUNT(ped.num_ped) AS 'Quantidade de Pedidos'
FROM
    tbFuncionarios AS func
    INNER JOIN tbPedidos AS ped ON func.cod_func = ped.cod_func
GROUP BY
    func.nome_func;

-- 5.Exiba quantos dependentes tem cada funcionário. Resposta: 
SELECT
    func.nome_func AS 'Funcionário',
    COUNT(dep.cod_dep) AS 'Quantidade de Dependentes'
FROM
    tbFuncionarios AS func
...
-- 9.Exiba quantos pedidos cada funcionário atendeu, mostrando o nome dos funcionários. Resposta: 
SELECT
    func.nome_func AS 'Funcionário',
    COUNT(ped.num_ped) AS 'Quantidade de Pedidos'
FROM
...
-- 10.Exiba quantos dependentes cada funcionário possui, mostrando seus nomes. Resposta:
SELECT
    func.nome_func AS 'Funcionário',
    COUNT(dep.cod_dep) AS 'Quantidade de Dependentes'
FROM
